...
for ($i = 1; $i <= $max; $i++) {
 ...
 // TRIMESTRE
	...
	if ( $a1[$valor] == TRUE ) {
		if ( $a2[$valor] == TRUE ) {
			if ( $a3[$valor] == TRUE ) {
				// print $valor." ".$fecha[$valor]." ".$proveedor[$valor]." ".$factura[$valor]." ".$detalle[$valor]." ".$total[$valor]." ".$pagada[$valor]."<br />";
				print ("Línea ".$valor." Cargada Correctamente");
				mysql_query("insert into gastos_s_iva (ano, trimestre, fecha, proveedor_id, nro_factura, detalle, total, pagada)
					values ('$ano[$valor]', '$trim[$valor]', '$fecha[$valor]', '$proveedor[$valor]', '$factura[$valor]', '$detalle[$valor]', '$total[$valor]', '$pagada[$valor]')");
				
			} else{
				print("<br />\nCampo de Total Obligatorio! No se cargan Datos de la Línea ".$valor."<br />\n");
			}
		} else{
...
?>
<br />
<a href="gastos_s_iva.php" class="botonR">Volver</a>
