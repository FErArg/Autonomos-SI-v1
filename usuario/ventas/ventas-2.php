	...
	if ( $a1[$valor] == TRUE ) {
		if ( $a2[$valor] == TRUE ) {
			if ( $a3[$valor] == TRUE ) {
				...
				// print $valor." ".$fecha[$valor]." ".$cliente[$valor]." ".$factura[$valor]." ".$detalle[$valor]." ".$base[$valor]." ".$iva[$valor]." ".$total[$valor]." ".$cobrada[$valor]."<br />";
				print ("Línea ".$valor." Cargada Correctamente");
				mysql_query("insert into ventas (ano, trimestre, fecha, cliente_id, nro_factura, detalle, base, iva, total, cobrada)
					values ('$ano[$valor]', '$trim[$valor]', '$fecha[$valor]', '$cliente[$valor]', '$factura[$valor]', '$detalle[$valor]', '$base[$valor]', '$iva[$valor]', '$total[$valor]', '$cobrada[$valor]')");
		...
		print("<br />\nCampo de Cliente Obligatorio! No se cargan Datos de la Línea ".$valor."<br />\n");
	}

}
?>
<br />
<a href="ventas.php" class="botonR">Volver</a>
